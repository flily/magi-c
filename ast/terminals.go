...
func (l *StringLiteral) EqualTo(_ context.ContextProvider, other Comparable) error {
	...
	if o.Value != l.Value {
		return l.Context().Error("wrong string value, expect '%s', got '%s'", o.Value, l.Value).With(o.Value)
	}
...
func (l *IntegerLiteral) EqualTo(_ context.ContextProvider, other Comparable) error {
	...
	if o.Value != l.Value {
		return l.Context().Error("wrong integer value, expect %v, got %v", o.Value, l.Value).With("%v", o.Value)
	}
...
func (l *FloatLiteral) EqualTo(_ context.ContextProvider, other Comparable) error {
	...
	if o.Value != l.Value {
		return l.Context().Error("wrong float value, expect %v, got %v", o.Value, l.Value).With("%v", o.Value)
	}
...
func (i *Identifier) EqualTo(_ context.ContextProvider, other Comparable) error {
	...
	if o.Name != i.Name {
		return i.Context().Error("wrong identifier name, expect '%s', got '%s'", o.Name, i.Name).With(o.Name)
	}
...
func (t *TerminalToken) EqualTo(_ context.ContextProvider, other Comparable) error {
	...
	if o.Token != t.Token {
		return t.Context().Error("wrong token type, expect '%v', got '%v'", o.Token, t.Token).With("%s", o.Token)
	}
